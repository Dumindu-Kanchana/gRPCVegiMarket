...
func main() {
	...
	defer cancel()
	response, err := c.UpdateVegetable(ctx, &pb.VegetableInfo{Name: vegiName, Amount: float32(amount), Price: float32(price)})
	if err != nil {
		log.Fatalf("could not obtain update vegitable response: %v", err)
	}
